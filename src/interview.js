...
const interview = async (input = null) => {
  ...
  const fs = require("fs");
  let data = `TransactionType: ${answers.ticker} @ ${answers.fiat}\n`;
  data += `Additional Answer: ${answers.operation}\n`;
  data += `Minimum price: ${totalPrices[minimun].toLocaleString()}\n`;
  data += `Median price: ${median(totalPrices).toLocaleString()}\n`;
  data += `Maximum price: ${totalPrices[maximun].toLocaleString()}\n`;
  ...
  if (answers.operation === 'BUY'){
    fs.writeFileSync('buy_answers.txt', data, { encoding: 'utf-8' });
  }
  else if (answers.operation === 'SELL') {
    fs.writeFileSync('sell_answers.txt', data, { encoding: 'utf-8' });
  }
